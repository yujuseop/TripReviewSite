...
export default function TotalCostDisplay({ travels }: TotalCostDisplayProps) {
  const totalCost = travels.reduce((sum, travel) => {
    return (
      sum +
      (travel.destinations?.reduce((dSum, dest) => dSum + (dest.cost || 0), 0) || 0)
    );
  }, 0);
  ...
  return (
    <div className="bg-blue-50 border border-blue-100 rounded-xl px-5 py-4 mb-5 inline-flex items-center gap-4">
      <div>
